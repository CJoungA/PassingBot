...
public enum ImpactType { Normal=0,Enemy, EnemyHead, EnemyArm, EnemyLeg }
...
public class ImpactMemoryPool : MonoBehaviour
{
    ...
    public void SpawnImpact(RaycastHit hit)
    {
        ...
        {
            OnSpawnImpact(ImpactType.EnemyHead, hit.point, Quaternion.LookRotation(hit.normal));
        }
        else if (hit.transform.CompareTag("ImpactEnemyArm"))
        {
            OnSpawnImpact(ImpactType.EnemyArm, hit.point, Quaternion.LookRotation(hit.normal));
        }
        else if (hit.transform.CompareTag("ImpactEnemyLeg"))
        {
            OnSpawnImpact(ImpactType.EnemyLeg, hit.point, Quaternion.LookRotation(hit.normal));
        }
    }
